...
class UserRepository {
  ...
  Future<dynamic> GoogleAuthUser(String? email) async {
    try {
      var res =
          await apiService.LoginSocialAuthUser(email) as Map<String, dynamic>;
      print(res['data']);
      if (res['data'] != null) {
        print("data here ");
      }
  ...
  Future<dynamic> AuthUserSignUp(fname, lname, String? email, String? password,
      telephone, username, gender) async {
    try {
      var res = await apiService.AuthSignUpUser(
          fname, lname, email, password, telephone, username, gender);
      print(res);
      if (res["error"]
          .toString()
          .contains("Email has already exist, try a new one")) {
        return res["error"];
      }
  ...
  Future<dynamic> SocialAuthRegister(
      {String? email, String? password, username, telephone}) async {
    try {
      var res = await apiService.AuthRegisterGoogle(
  ...
  //   }
  // }
  Future<dynamic> UserEditing(
      {fname, lname, telephone, username, gender, imgid, uid}) async {
    try {
      ...
      res = await apiService.UserEditing(
          fname, lname, telephone, username, gender, imgid, uid, url!);
